...
const CategoryPage: React.FC = () => {
    ...
    const [deleteCategory, setDeleteCategory] = useState<CategoryDTO | undefined>(undefined);
    ...
    const handleSave = async () => {
        try {
            const response = await CategoryService.updateCategory(editedCategory as CategoryDTO);
    ...
    const handleDelete = async () => {
        try {
            const response = await CategoryService.deleteCategoryByID(deleteCategory?.categoryId ?? '');
            if (response.isSuccess) {
                fetchCategories();
        ...
        <Layout>
                    ...
                    <tbody>
                        {currentCategories.map((category) => (
                            <tr key={category.categoryId}>
                                ...
                                <td>{category.type}</td>
                                <td>
                                    <button type="button" className="btn btn-primary" onClick={() => handleEdit(category)}>Edit</button>
                                    <button type="button" className="btn btn-danger ms-2" onClick={() => { setDeleteCategory(category); setConfirmDeleteModalOpen(true); }}>Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <Modal show={editModalOpen} onHide={() => setEditModalOpen(false)}>
                ...
                <Modal.Body>
                    ...
                    <div className="mb-3">
                        ...
                        <input type="text" className="form-control" id="name" value={editedCategory.categoryName} onChange={(e) => setEditedCategory({ ...editedCategory, categoryName: e.target.value })} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="type" className="form-label">Type</label>
                        <input type="text" className="form-control" id="type" value={editedCategory.type} onChange={(e) => setEditedCategory({ ...editedCategory, type: e.target.value })} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="quantity" className="form-label">Quantity</label>
                        <input type="number" className="form-control" id="quantity" value={editedCategory.quantity} onChange={(e) => setEditedCategory({ ...editedCategory, quantity: e.target.value })} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="note" className="form-label">Note</label>
                        <textarea className="form-control" id="note" value={editedCategory.note} onChange={(e) => setEditedCategory({ ...editedCategory, note: e.target.value })}></textarea>
                    </div>
            ...
            <Modal show={addModalOpen} onHide={() => setAddModalOpen(false)}>
                ...
                <Modal.Body>
                    ...
                    <div className="mb-3">
                        <label htmlFor="name" className="form-label">Name</label>
                        <input type="text" className="form-control" id="name" value={newCategory.categoryName} onChange={(e) => setNewCategory({ ...newCategory, categoryName: e.target.value })} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="type" className="form-label">Type</label>
                        <input type="text" className="form-control" id="type" value={newCategory.type} onChange={(e) => setNewCategory({ ...newCategory, type: e.target.value })} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="quantity" className="form-label">Quantity</label>
                        <input type="number" className="form-control" id="quantity" value={newCategory.quantity} onChange={(e) => setNewCategory({ ...newCategory, quantity: e.target.value })} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="note" className="form-label">Note</label>
                        <textarea className="form-control" id="note" value={newCategory.note} onChange={(e) => setNewCategory({ ...newCategory, note: e.target.value })}></textarea>
                    </div>
                    {/* Add other form fields here */}
                </Modal.Body>
                <Modal.Footer>
