...
const Post: React.FC = () => {
    ...
    const handleDelete = async () => {
        try {
            const response = await PostService.deletePostByID(deletePost?.postId ?? '');
            if (response.isSuccess) {
                fetchPosts();
        ...
        <Layout>
                    ...
                    <tbody>
                        {currentPosts.map((post) => (
                            <tr key={post.postId}>
                                ...
                                <td>{post.status}</td>
                                <td>
                                    <button type="button" className="btn btn-primary" onClick={() => handleEdit(post)}>Edit</button>
                                    <button type="button" className="btn btn-danger ms-2" onClick={() => { setDeletePost(post); setConfirmDeleteModalOpen(true); }}>Delete</button>
                                </td>
                            </tr>
                        ))}
                    </tbody>
            ...
            <Modal show={editModalOpen} onHide={() => setEditModalOpen(false)}>
                ...
                <Modal.Body>
                    ...
                    <div className="mb-3">
                        ...
                        <input type="text" className="form-control" id="postId" value={editedPost.postId} onChange={(e) => setEditedPost({ ...editedPost, postId: e.target.value })} readOnly />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="artworkId" className="form-label">Artwork ID</label>
                        <input type="text" className="form-control" id="artworkId" value={editedPost.artworkId} onChange={(e) => setEditedPost({ ...editedPost, artworkId: e.target.value })} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="title" className="form-label">Title</label>
                        <input type="text" className="form-control" id="title" value={editedPost.tittle} onChange={(e) => setEditedPost({ ...editedPost, title: e.target.value })} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="description" className="form-label">Description</label>
                        <textarea className="form-control" id="description" value={editedPost.description} onChange={(e) => setEditedPost({ ...editedPost, description: e.target.value })}></textarea>
                    </div>
                    <div className="mb-3">
                        <label htmlFor="createdOn" className="form-label">Created On</label>
                        <input type="text" className="form-control" id="createdOn" value={editedPost.createdOn} onChange={(e) => setEditedPost({ ...editedPost, createdOn: e.target.value })} readOnly />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="status" className="form-label">Status</label>
                        <select className="form-select" id="status" value={editedPost.status} onChange={(e) => setEditedPost({ ...editedPost, status: e.target.value })}>
            ...
            <Modal show={addModalOpen} onHide={() => setAddModalOpen(false)}>
                ...
                <Modal.Body>
                    ...
                    <div className="mb-3">
                        <label htmlFor="artworkId" className="form-label">Artwork ID</label>
                        <input type="text" className="form-control" id="artworkId" value={newPost.artworkId} onChange={(e) => setNewPost({ ...newPost, artworkId: e.target.value })} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="title" className="form-label">Title</label>
                        <input type="text" className="form-control" id="title" value={newPost.title} onChange={(e) => setNewPost({ ...newPost, title: e.target.value })} />
                    </div>
                    <div className="mb-3">
                        <label htmlFor="description" className="form-label">Description</label>
                        <textarea className="form-control" id="description" value={newPost.description} onChange={(e) => setNewPost({ ...newPost, description: e.target.value })}></textarea>
                    </div>
                    <div className="mb-3">
                        <label htmlFor="status" className="form-label">Status</label>
                        <select className="form-select" id="status" value={newPost.status} onChange={(e) => setNewPost({ ...newPost, status: e.target.value })}>
                            <option value="Published">Published</option>
                            <option value="Draft">Draft</option>
                        </select>
                    </div>
                ...
                <Modal.Footer>
                    ...
                    <Button variant="primary" onClick={handleAdd}>Add Post</Button>
                </Modal.Footer>
            </Modal>
