...
const Home: React.FC = () => {
  ...
  const [currentConfigurations, setCurrentConfigurations] = useState<ConfigurationDTO[]>([]);

  const [editedConfiguration, setEditedConfiguration] = useState<Partial<ConfigurationDTO>>({});
  ...
  const [deleteConfiguration, setDeleteConfiguration] = useState<ConfigurationDTO | undefined>(undefined);
  ...
  useEffect(() => {
    ...
    setCurrentConfigurations(configurations.slice(indexOfFirstItem, indexOfLastItem));
  }, [currentPage]);
  ...
  const handleSave = async () => {
    try {
      const response = await ConfigurationService.updateConfiguration(editedConfiguration as ConfigurationDTO);
  ...
  const handleDelete = async () => {
    try {
      const response = await ConfigurationService.deleteConfigurationByID(deleteConfiguration?.id ?? '', true);
      if (response.isSuccess) {
        fetchConfigurations();
    ...
    <Layout>
          ...
          <tbody>
            {currentConfigurations.map((config) => (
              <tr key={config.configurationId}>
                ...
                <td>{config.status}</td>
                <td>
                  <button type="button" className="btn btn-primary" onClick={() => handleEdit(config)}>Edit</button>
                  <button type="button" className="btn btn-danger ms-2" onClick={() => { setDeleteConfiguration(config); setConfirmDeleteModalOpen(true); }}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <Modal show={editModalOpen} onHide={() => setEditModalOpen(false)}>
        ...
        <Modal.Body>
          ...
          <div className="mb-3">
            <label htmlFor="commissionFee" className="form-label">Commission Fee</label>
            <input type="number" className="form-control" id="commissionFee" value={editedConfiguration.commisionFee} onChange={(e) => setEditedConfiguration({ ...editedConfiguration, commissionFee: e.target.value })} />
          </div>
          <div className="mb-3">
            <label htmlFor="appliedDate" className="form-label">Applied Date</label>
            <input type="date" className="form-control" id="appliedDate" value={editedConfiguration.appliedDate} onChange={(e) => setEditedConfiguration({ ...editedConfiguration, appliedDate: e.target.value })} />
          </div>
          <div className="mb-3">
            <label htmlFor="status" className="form-label">Status</label>
            <select className="form-select" id="status" value={editedConfiguration.status} onChange={(e) => setEditedConfiguration({ ...editedConfiguration, status: e.target.value })}>
      ...
      <Modal show={addModalOpen} onHide={() => setAddModalOpen(false)}>
        ...
        <Modal.Body>
          ...
          <div className="mb-3">
            <label htmlFor="commissionFee" className="form-label">Commission Fee</label>
            <input type="number" className="form-control" id="commissionFee" value={newConfiguration.commissionFee} onChange={(e) => setNewConfiguration({ ...newConfiguration, commissionFee: e.target.value })} />
          </div>
          <div className="mb-3">
            <label htmlFor="appliedDate" className="form-label">Applied Date</label>
            <input type="date" className="form-control" id="appliedDate" value={newConfiguration.appliedDate} onChange={(e) => setNewConfiguration({ ...newConfiguration, appliedDate: e.target.value })} />
          </div>
          <div className="mb-3">
            <label htmlFor="status" className="form-label">Status</label>
            <select className="form-select" id="status" value={newConfiguration.status} onChange={(e) => setNewConfiguration({ ...newConfiguration, status: e.target.value })}>
              <option value="Active">Active</option>
              <option value="Inactive">Inactive</option>
            </select>
          </div>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" onClick={handleAdd}>Add Configuration</Button>
        </Modal.Footer>
      </Modal>
